...
/**
 * 可以指定排序列的RecordComparator。排序策略是自然序。
 */
public class LocalColumnBasedRecordComparator extends NaturalRecordComparator {
   ...
   *     整个Record的行属性
   */
  public LocalColumnBasedRecordComparator(int[] selIdxs, Column[] schema, SortOrder[] sortOrders) {
    super(schema);
  ...
  /**
   * ColumnBasedRecordComparator的构造方法，指定排序列的列名。例如指定排序列的下标索引为“word”，“count”，
   * 则比较时先用word列比较， word列相同时使用count列比较。
   *
   * @param selCols
   * @param schema
   */
  public LocalColumnBasedRecordComparator(String[] selCols, Column[] schema,
                                          SortOrder[] sortOrders) {
    super(schema);
