...
/**
 * This class implements the common functionalities of the subclasses of
 * ValueAggregatorDescriptor class.
 */
public class ValueAggregatorBaseDescriptor implements ValueAggregatorDescriptor {
   ...
   * @return an Entry whose key is the aggregation id prefixed with the
   * aggregation type.
   */
  public static Entry<String, String> generateEntry(String type, String id,
                                                    String val) {
    String key = new String(type + TYPE_SEPARATOR + id);
   ...
   * The first id will be of type LONG_VALUE_SUM, with "record_count" as its
   * aggregation id. If the input is a file split, the second id of the same
   * type will be generated too, with the file name as its aggregation id. This
   * achieves the behavior of counting the total number of records in the input
   * data, and the number of records in each input file.
   *
   * @param key
   *     input key
   * @param val
  ...
  public ArrayList<Entry<String, String>> generateKeyValPairs(Object key,
    ...
    Entry<String, String> e = generateEntry(countType, id, ONE);
    if (e != null) {
      retv.add(e);
    }
