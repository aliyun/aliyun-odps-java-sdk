...
/**
 * {@link ArrowFieldWriter} for DateDay.
 */
public abstract class ArrowDateDayWriter<IN> extends ArrowFieldWriter<IN> {
    ...
    protected void setNull() {
        ((DateDayVector) valueVector).setNull(count);
    }

    @Override
    protected void setValue(IN row, int ordinal) {
        ((DateDayVector) valueVector).setSafe(count, readEpochDay(row, ordinal));
    }

    abstract protected int readEpochDay(IN row, int ordinal);
}
