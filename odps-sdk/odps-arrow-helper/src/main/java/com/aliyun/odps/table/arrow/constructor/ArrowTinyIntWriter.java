...
public abstract class ArrowTinyIntWriter<IN> extends ArrowFieldWriter<IN> {
    ...
    protected void setNull() {
        ((TinyIntVector) valueVector).setNull(count);
    }

    @Override
    protected void setValue(IN row, int ordinal) {
        ((TinyIntVector) valueVector).setSafe(count, readByte(row, ordinal));
    }

    abstract protected byte readByte(IN row, int ordinal);
}
