...
public abstract class ArrowTimeStampWriter<IN> extends ArrowFieldWriter<IN> {
    ...
    protected void setNull() {
        ((TimeStampVector) valueVector).setNull(count);
    }

    @Override
    protected void setValue(IN row, int ordinal) {
        ((TimeStampVector) valueVector).setSafe(count, readEpochTime(row, ordinal));
    }

    public ArrowType.Timestamp getType() {
        return (ArrowType.Timestamp) valueVector.getField().getType();
    }
