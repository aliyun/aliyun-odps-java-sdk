...
public class ArrowArrayAccessorImpl {
    ...
    public static class ArrowArrayAccessorForRecord extends ArrowArrayAccessor<List<Object>> {
        ...
        public ArrowArrayAccessorForRecord(ListVector vector, TypeInfo typeInfo, boolean isExtension, boolean encodeString) {
            super(vector);
            this.isExtension = isExtension;
            this.encodeString = encodeString;
            this.elementTypeInfo = ((ArrayTypeInfo) typeInfo).getElementTypeInfo();
            this.dataAccessor = ArrowToRecordConverter.
                    createColumnVectorAccessor(vector.getDataVector(), elementTypeInfo, isExtension, encodeString);
        }
            ...
            try {
                for (int i = 0; i < length; i++) {
                    list.add(ArrowToRecordConverter.getData(dataAccessor,
                            elementTypeInfo, offset + i, isExtension, encodeString));
                }
                return list;
            } catch (Exception e) {
