...
import com.aliyun.odps.data.JsonString;
...
public class ArrowToRecordConverter {
  ...
  public static ArrowVectorAccessor createColumnVectorAccessor(ValueVector vector,
                                                               TypeInfo typeInfo,
                                                               boolean isExtension) {
    return createColumnVectorAccessor(vector, typeInfo, isExtension, false);
  }
   ...
   * @param encodeString 是否将STRING类型的数据进行编码, 默认为false，即 String 类型返回 byte[], 如配置成 true，则使用 utf-8 编码返回 String
   */
  public static ArrowVectorAccessor createColumnVectorAccessor(ValueVector vector,
                                                               TypeInfo typeInfo,
                                                               boolean isExtension,
                                                               boolean encodeString) {
  ...
  public static Object getData(ArrowVectorAccessor dataAccessor, TypeInfo typeInfo, int rowId,
                               boolean isExtension) throws IOException {
    return getData(dataAccessor, typeInfo, rowId, isExtension, false);
  }

  public static Object getData(ArrowVectorAccessor dataAccessor, TypeInfo typeInfo, int rowId,
                               boolean isExtension, boolean encodeString)
      throws IOException {
    ...
    switch (typeInfo.getOdpsType()) {
      ...
      case BLOB:
        return Blob.fromReference(new String(((ArrowVarBinaryAccessor) dataAccessor).getBinary(rowId),
