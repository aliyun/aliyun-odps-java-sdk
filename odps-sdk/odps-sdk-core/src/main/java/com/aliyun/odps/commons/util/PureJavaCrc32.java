...
/**
 * A pure-java implementation of the CRC32 checksum that uses the same
 * polynomial as the built-in native CRC32.
 *
 * This is to avoid the JNI overhead for certain uses of Checksumming where many
 * small pieces of data are checksummed in succession.
 *
...
public class PureJavaCrc32 implements Checksum {
  ...
  public void update(byte[] b, int off, int len) {
    ...
    switch (len) {
      case 7:
        localCrc = (localCrc >>> 8)
                   ^ T[T8_0_start + ((localCrc ^ b[off++]) & 0xff)];
      case 6:
        localCrc = (localCrc >>> 8)
                   ^ T[T8_0_start + ((localCrc ^ b[off++]) & 0xff)];
      case 5:
  ...
  /*
   * CRC-32 lookup tables generated by the polynomial 0xEDB88320. See also
   * TestPureJavaCrc32.Table.
   */
