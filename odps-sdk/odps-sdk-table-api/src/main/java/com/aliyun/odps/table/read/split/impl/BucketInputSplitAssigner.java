...
public class BucketInputSplitAssigner implements InputSplitAssigner {

    private final int totalSplitCount;

    private final List<Integer> splitBucketId;

    private final String sessionId;

    public BucketInputSplitAssigner(String sessionId,
                                    int totalSplitCount,
                                    List<Integer> splitBucketId) {
    ...
    public InputSplit getSplit(int index) {
        ...
        return new BucketInputSplit(sessionId, index, splitBucketId.get(index));
    }

    @Override
    public InputSplit[] getAllSplits() {
        BucketInputSplit[] splits = new BucketInputSplit[totalSplitCount];
        for (int i = 0; i < this.totalSplitCount; i++) {
            splits[i] = new BucketInputSplit(sessionId, i, splitBucketId.get(i));
        }
        return splits;
    }
}
