 ...
 * <p>
 * This policy retries up to a maximum number of times with exponential backoff,
 * and adds random jitter to prevent thundering herd problems when multiple clients
 * retry simultaneously.
 * <p>
 * Wait time calculation: base_wait = 2^(attempt-1) seconds
...
public class ExponentialBackoffPolicy implements RetryPolicy {
  ...
  public long getRetryWaitTime(int attempt) {
    long baseWaitMs = (long) Math.pow(2, attempt - 1) * 1000;
    long jitterMs = (long) (baseWaitMs * JITTER_FACTOR * Math.random());
    return Math.min(baseWaitMs + jitterMs, MAX_WAIT_TIME_MS);
  }
}